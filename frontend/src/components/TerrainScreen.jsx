...
export default function TerrainScreen() {
  ...
  const [sidePanel, setSidePanel] = useState('guide') // 'guide' | 'info'
    ...
    <div style={styles.root}>
      ...
      {showControls && (
        <div style={styles.controlsOverlay} onClick={() => setShowControls(false)}>
          <div style={styles.controlsCard} onClick={e => e.stopPropagation()}>
            <p style={styles.controlsTitle}>FLIGHT CONTROLS</p>
            <table style={styles.controlsTable}>
              <tbody>
                {[
                  ...
                  ['E', 'Ascend'],
                  ['Q', 'Descend'],
                  ['Drag', 'Orbit (non-fly mode)'],
                  ['Scroll', 'Zoom'],
                ].map(([key, desc]) => (
                  <tr key={key}>
                    <td style={styles.keyCell}><kbd style={styles.kbd}>{key}</kbd></td>
                    <td style={styles.descCell}>{desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <button style={{...styles.hudBtn, marginTop: '1rem', width: '100%'}} onClick={() => setShowControls(false)}>
              CLOSE
            </button>
          </div>
        </div>
      ...
      {flyMode && (
        <div style={styles.posReadout}>
          <span style={styles.posLabel}>POS</span>
          <span>{cameraPosition.x.toFixed(1)}, {cameraPosition.y.toFixed(1)}, {cameraPosition.z.toFixed(1)}</span>
        </div>
      )}
...
const styles = {
  ...
  posReadout: {
    ...
    fontFamily: 'Cinzel, serif', fontSize: '0.55rem',
    letterSpacing: '0.15em', color: '#6a5038',
    background: 'rgba(13,11,8,0.7)',
  ...
  keyCell: { padding: '0.35rem 0.75rem 0.35rem 0', width: 90 },
  descCell: {
    fontFamily: 'Crimson Pro, serif', fontSize: '0.9rem',
    color: '#8a7868', padding: '0.35rem 0',
  },
  kbd: {
    fontFamily: 'Cinzel, serif', fontSize: '0.6rem',
